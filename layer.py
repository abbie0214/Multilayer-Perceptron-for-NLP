...
class WordEmbedding:
    def __init__(self, vocab_size, embedding_dim, init_wt=0.01, weight=None):
        ...
        self.grad_weight = torch.zeros(vocab_size, embedding_dim) # 250 * 16
    ...
    def backward(self, grad_output):
        self.grad_weight = torch.zeros(self.vocab_size, self.embedding_dim)
        for i in range(self.input.size(1)):
            self.get_indicator_matrix(self.input[:, i].long())
            self.grad_weight = self.grad_weight + \
                torch.mm(torch.t(self.indicator_matrix), grad_output[:, i*self.embedding_dim:(i + 1) * self.embedding_dim])
...
class Linear:
    ...
    def backward(self, grad_output):
        self.grad_weight = torch.add(self.grad_weight, torch.mm(torch.t(self.input), grad_output))
        self.grad_bias = torch.add(self.grad_bias, torch.sum(grad_output, 0))
        grad_output = torch.mm(grad_output, torch.t(self.weight))
        return grad_output
...
class Sigmoid():
    ...
    def backward(self, grad_output):
        grad = torch.mul(self.input, 1 - self.input)
        return torch.mul(grad, grad_output)
